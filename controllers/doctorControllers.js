...
const getRegistrationPage = (req, res) => {
    res.sendFile(path.join(__dirname, '..', 'public', 'html', 'doctor_reg.html'));
};
...
const register = async (req, res) => {
    ...
    if (!fnameCorrect) {
        ...
        // Generate Doctor ID
        let doctorNum = generateRandomNum(5);
        const dateObj = new Date();
        const doctorYear = dateObj.getFullYear();
        const doctorID = pharmacyID + "D" + doctorNum + doctorYear;
        const fname = titleCase(req.body.fname);
        const lname = titleCase(req.body.lname);

        // Insert data into the DB
        const statement1 = `USE PharmacyDB`;
        const statement2 = `INSERT INTO doctors VALUES ('${doctorID}', '${fname}', '${lname}', '${req.body.email}', '${req.body.mobile}', '${req.body.dob}', '${req.body.gender[0].toUpperCase()}', '${req.body.address}');`;
